...
{
  oled.clearDisplay();
  display_pre_weather(1, 5, 0); // 1 type tomorrow,2:after tomorrow
  display_pre_weather(2, 65, 0);
  oled.display();
  delay(40);
}
...
void rgb_second_display()
{
  ...
  if (rgb_iscolor)
  {
    ...
    case 0:
      rgb_color = 1;
    case 1:
      OLED_ShowString(48, 30, 1, "random");
      rgb_R = 169, rgb_G = 0, rgb_B = 56;
      break;
    case 2:
      OLED_ShowString(48, 30, 1, "warm");
      rgb_R = 169, rgb_G = 101, rgb_B = 56;
      break;
    case 3:
      OLED_ShowString(48, 30, 1, "white");
    ...
    OLED_ShowString(20, 30, 1, "<");
    OLED_ShowString(100, 30, 1, ">");
  }
  ...
  else if (rgb_istype)
  {
    ...
    case 0:
      rgb_type = 4;
    case 1:
      OLED_ShowString(45, 30, 1, "normal");
      break;
    case 2:
      OLED_ShowString(45, 30, 1, "breathe");
      break;
    case 3:
      OLED_ShowString(45, 30, 1, "rainbow");
      break;
    case 4:
      OLED_ShowString(45, 30, 1, "warning");
      OLED_ShowString(45, 40, 1, "light");
      break;
    case 5:
      rgb_type = 1;
      break;
...
void Wifi_web_display()
{
  ...
  {
    OLED_ShowString(4, 53, 1, "ON_CONNECT_WIFI");
    oled.drawBitmap(42, 5, f_cube[frame_s], FRAME_WIDTH_48, FRAME_HEIGHT_48, 1);
  }
  else
  {
...
void Wifi_web_second_display()
{
  ...
  if (!C_new_wifi)
  {
    ...
    case 1:
      ...
      {
        oled.clearDisplay();
        OLED_ShowString(3, 12, 2, "Wsit 10s");
        OLED_ShowString(3, 32, 2, "CONNECT...");
        oled.display();
        Wifi_connect(0);
        if (WiFi.status() == WL_CONNECTED)
        {
    ...
    OLED_ShowString(60, 22, 1, "CMCC-FPC2");
    OLED_ShowString(60, 32, 1, wifiName);
    OLED_ShowString(5, 42, 1, "connect_NEW_wifi");
  }
...
void Mqtt_display()
{
  ...
  {
    OLED_ShowString(4, 53, 1, "ON_CONNECT_MQTT");
    oled.drawBitmap(42, 5, f_cube[frame_s], FRAME_WIDTH_48, FRAME_HEIGHT_48, 1);
    frame_s = (frame_s + 1) % FRAME_COUNT_48;
  }
  else
  {
    OLED_ShowString(4, 53, 1, "CONNECT_WIFI_MQTT");
    oled.drawBitmap(42, 5, f_set[frame_s], FRAME_WIDTH_48, FRAME_HEIGHT_48, 1);
    frame_s = (frame_s + 1) % FRAME_COUNT_48;
    if (ScanKey(B_M_) == 1)
    {
...
if (mqttportdelay == 200)
  ...
  oled.clearDisplay();
  OLED_ShowString(4, 53, 1, "ON_CONNECT_MQTT");
  oled.drawBitmap(10, 8, f_yes_32[frame_s], FRAME_WIDTH, FRAME_WIDTH, 1);
  if (mqttClient.connected())
    oled.drawBitmap(10, 8, f_yes_32[frame_s], FRAME_WIDTH, FRAME_WIDTH, 1);
  else
    oled.drawBitmap(10, 8, f_error_32[frame_s], FRAME_WIDTH, FRAME_WIDTH, 1);
  frame_s = (frame_s + 1) % FRAME_COUNT_32;
  oled.drawRect(0, 0, 128, 64, 1);
  oled.drawRect(2, 2, 124, 60, 1);
  if (ScanKey(B_R_) == 1)
  ...
  if (ScanKey(B_L_) == 1)
    mqtt_index--;
  OLED_ShowString(50, 12, 1, "reconnect");
  OLED_ShowString(50, 22, 1, "update");
  OLED_ShowString(50, 32, 1, "logout");
  switch (mqtt_index)
  {
  case 0:
    mqtt_index = 3;
  ...
  if (ScanKey(B_M_) == 1)
    {
      ...
      //digitalWrite(JDQ_PIN, 0);
    }
    else{
      pinMode(JDQ_PIN, INPUT);
       //digitalWrite(JDQ_PIN, 1);
    }
  oled.display();
  frame_s = (frame_s + 1) % FRAME_COUNT_48;
  delay(40);
...
void diaplay_message()
{
  OLED_ShowString(2, 2, 1, "[WIFI");
  OLED_ShowString(2, 14, 1, "[MQTT");
  if (WiFi.status() == WL_CONNECTED){
    OLED_ShowString(35, 2, 1, "ON");
  }
else{ OLED_ShowString(35, 2, 1, "OFF");}
if (mqttClient.connected()){
 OLED_ShowString(35, 14, 1, "ON");
}
else{
   OLED_ShowString(35, 14, 1, "OFF");
}
  
 
  OLED_ShowString(2, 55, 1, "[FAN");
  OLED_ShowString(2, 45, 1, "[LIG");
  if(rgb_isopen){
    OLED_ShowString(30, 45, 1, "ON");
  }
  else{ OLED_ShowString(30, 45, 1, "OFF");}
  if(fan_isopen){
    OLED_ShowString(30, 55, 1, "ON");
  }
  else{OLED_ShowString(30, 55, 1, "OFF");}
...
void All_message_display(){
  ...
  oled.drawBitmap(90, 0, frames_skate_board[frame_s], FRAME_WIDTH, FRAME_HEIGHT, 1);
  frame_s = (frame_s + 1) % FRAME_COUNT_32;

  OLED_ShowString(100, 45, 1, "T:");
  OLED_ShowString(100, 55, 1, "H:");
  OLED_ShowNum(112, 45, 1, _Temperature);
  OLED_ShowNum(112, 55, 1, _Humidity);
...
void all_message_second_display(){
